...
internal class PoolPolicy<T> where T : class
{
    ...
    public uint MaximumPoolSize
    {
        ...
        set => _maximumPoolSize = value > 0
            ? value
            : throw new PoolingException($"Policy for {nameof(MaximumPoolSize)} size must be greater than 0",
                typeof(T));
    }

    /// <summary>
    /// The initial capacity the item store will be created with. May not be used in <see cref="IObjectPool{T}" />
    /// implementations.
    /// Default is 10.
    /// </summary>
    ...
    ///     the pool that was asked for the object, useful if you're creating <see cref="PooledObject{T}" /> instances.
    ///     </para>
    ///     <para>
    ///     May not be <see langword="null" />. If <see langword="null" /> when provided to an
    ///     <see cref="IObjectPool{T}" /> instance, an <see cref="ArgumentNullException" /> will be thrown.
    ///     </para>
    /// </remarks>
